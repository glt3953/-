# 链表
## 链表反转
链表的反转就是改变每个节点的 next 指针，使其指向前一个节点，而不是后一个节点。
### 代码示例
该算法的实现方法如下：
* 如果链表为空，或者只有一个节点，则直接返回原链表。
* 创建两个指针，分别指向前一个节点和当前节点，然后遍历整个链表进行反转。
* 返回新的头节点。

该算法的时间复杂度是O(n)，其中n是链表的长度。你需要遍历整个链表一次，空间复杂度是 O(1)，因为你只使用了几个额外的变量。
```
...
- (ListNode *)reverseLinkedList:(ListNode *)head {
    ...
    ListNode *prev = nil; // 记录当前节点的前一个节点
    ListNode *current = head; // 记录当前节点
    
    // 遍历链表进行反转
    while (current != nil) {
        ListNode *next = current.next; // 记录当前节点的下一个节点
...
- (void)delete:(NSInteger)data
{
    ...
    if (currentNode == self.head) {
        //如果删除的是头节点，那么就会找到尾部节点，让尾部节点的 next 指向头节点的下一个节点。
        ListNode *temp = self.head;
        while (temp.next != self.head) {
            temp = temp.next;
        }
        temp.next = self.head.next;
        self.head = self.head.next; //重新设置头节点
    } else {
        //如果删除的不是头节点，那么直接让前一个节点的 next 指向当前节点的下一个节点，从而删除当前节点。
...
该算法的实现方法如下：
* 如果链表为空或只有一个节点，则肯定不可能存在循环。
* 用两个指针slow和fast分别初始指向头节点，然后遍历链表进行判断。
...
- (BOOL)hasCycle:(ListNode *)head {
    ...
    ListNode *slow = head; // 慢指针初始指向头节点
    ListNode *fast = head.next; // 快指针初始指向头节点的下一个节点
